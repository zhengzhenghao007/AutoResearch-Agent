...
### P0 离线审核工具（2026-09-24）

- [x] 导出独立审核包：原 PDF、逐页文本、来源许可与哈希、页数范围以及空白人工审核表。
- [x] 导入时核对原始清单、PDF 哈希、审核元数据和引文位置；输出独立评测副本及审计文件，拒绝覆盖已有目录。
- [ ] 真实审核人完成标注后才可获得人工评测结果；工具和临时测试数据不替代此门槛。操作说明见 `tests/fixtures/evidence/README.md`。

### P2 首个增量：arXiv 候选规范化（2026-09-26）

- [x] 新搜索结果将同一版本的 HTTP/HTTPS、abs/pdf 路径及可选 `.pdf` 后缀统一为规范 PDF 链接，重复候选保留首次出现的元数据。
- [x] 未指定版本、v1、v2 分别保留；不根据标题相似度合并，也不推断 DOI 与 arXiv 的对应关系。
- [x] 未识别或含查询、片段、凭据、端口的链接不做宽松修复，下载仍受原有白名单限制；历史研究记录不迁移。
- [ ] 完整引文注册表、DOI/BibTeX、多来源去重、纳入排除记录和语义审核仍属于后续工作。P0 人工评测门槛保持未完成。

### 外部实现复核完成（2026-09-26）

- [x] ChatGPT 已恢复项目只读连接，并对代码提交 `8e21e82` 给出实现复核 DONE；未发现阻塞问题。复核依据及工具限制见 `docs/p0-p1-implementation.md`。
- [ ] 真实人工科学评测与合并验收仍未完成；PR 继续保留草稿状态。

### P2 只读引文清单

- [x] 从已校验的 ResearchRun 派生来源哈希、原文证据、论文结论及建议引用；API/CLI 共用实现，旧记录无需迁移。
- [x] 仅在原始链接明确带版本号时输出 arXiv revision；未指定版本保持未知，不推断 DOI 或 BibTeX。
- [ ] 完整文献身份注册、纳入排除记录、语义审核和真实人工评测仍待后续完成。使用说明见 `docs/p2-citation-registry.md`。
