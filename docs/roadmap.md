...
只生成结构化 suggestion，不主动生成 hypothesis；用户实验输入与完整写作在后续里程碑。旧前端保持原样，新增流程通过 CLI/API 使用。JSON 为单用户快照，不支持并发修改同一个 run 的事务隔离。
...
- [ ] 建立 CI：新环境安装明确的测试依赖；默认不注入 provider key，不运行 `--run-live`；失败即阻止集成。
- [ ] 建立小型可再分发评测集，涵盖正常文字、扫描页、截断、未报告实验、计划实验及不支持的引用；保存人工标签和来源许可。
- [ ] 用失败测试定义研究历史列表与详情契约，再增加最小实现；确认旧接口响应兼容。
- [ ] 实现搜索 → 显式选择 → 分析的页面链路；长任务必须有可辨认的状态，不能把超时当成功。
- [ ] 实现上传 → 同一研究记录 → 比较；加入文件重复、解析失败、空证据和刷新恢复测试。
- [ ] 实现 evidence 面板：原文、页码、来源、claim_type、缺失类别、截断提示；建议与论文原话有独立标签。
- [ ] 完成真实用户流程验证和独立代码审查后再合并；发布说明保留已知限制。
